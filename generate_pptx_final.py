"""
MAIR+++ — PowerPoint Presentation Generator
============================================
...
txt(s, "MAIR+++", 0.5, 1.05, 12, 0.9, size=52, bold=True, color=WHITE)
...
section_header_box(s, "Overview", "Executive Summary", "What MAIR+++ builds, why it matters, and what it achieves in Image Restoration")
...
txt(s, "MAIR+++ operates in Era 3, overcoming the rigidity of single models by chaining experts adaptively.", 0.4, 7.0, 12, 0.4, size=12, bold=True, color=WHITE)
...
txt(s, "These fundamental LLM bottlenecks necessitated the development of MAIR+++.", 0.4, 7.0, 12, 0.4, size=12, bold=True, color=WHITE)
...
# ═══════════════════════════════════════════════════════════════
s = add_slide(); bg(s); accent_bar(s, ACCENT_BLUE)
section_header_box(s, "Direct Comparison", "MAIR vs MAIR+++", "Transforming an experimental LLM pipeline into a deployable deterministic system")

table_data = [
    ("Feature", "Original MAIR (Jiang et al.)", "MAIR+++ (Our System)"),
    ...
    ("Benchmark: Noise", "Baseline SSIM", "Overall PSNR Gain: +2.74 dB"),
    ("Benchmark: Mixed", "Baseline SSIM", "Overall SSIM Gain: +0.088"),
]
...
# ═══════════════════════════════════════════════════════════════
s = add_slide(); bg(s); accent_bar(s, ACCENT_BLUE)
txt(s, "MAIR+++ — Complete Pipeline Architecture", 0.4, 0.2, 12.5, 0.6, size=24, bold=True, color=WHITE)
...
txt(s, "Highlighted experts added as MAIR+++ contributions.", 0.35, 7.12, 12.6, 0.28, size=10, color=TEXT_DIM, align=PP_ALIGN.CENTER)
...
# ═══════════════════════════════════════════════════════════════
s = add_slide(); bg(s); accent_bar(s, ACCENT_BLUE)
section_header_box(s, "Context", "Comparison with State-of-the-Art", "MAIR+++ vs. all major agentic systems")
...
systems_data = [
    ("MAIR+++ (Ours)","2026","7 ★","✅","✅","✅","0.77+",ACCENT_GREEN,True),
...
txt(s, "KEY MAIR+++ ADVANTAGES:", 0.55, 5.6, 12, 0.3, size=11, bold=True, color=ACCENT_GREEN)
...
box(s, 0.4, 1.6, 12.5, 5.0, BG_CARD, ACCENT_GREEN, 1.0)
txt(s, "Ablation A5: Full MAIR+++ vs. Base MAIR", 0.6, 1.8, 12, 0.4, size=20, bold=True, color=ACCENT_GREEN)
...
import os
img_path_rain = os.path.join("outputs", "comparison_grids", "lenna_rain_comparison.png")
if os.path.exists(img_path_rain):
    ...
    txt(s, f"[Image not found: {img_path_rain}]", 0.6, 2.2, 12, 0.4, size=14, color=ACCENT_RED)

img_path_blur = os.path.join("outputs", "comparison_grids", "butterfly_blur_comparison.png")
if os.path.exists(img_path_blur):
    ...
    txt(s, f"[Image not found: {img_path_blur}]", 0.6, 4.6, 12, 0.4, size=14, color=ACCENT_RED)

# ═══════════════════════════════════════════════════════════════
# 42. FUTURE WORK
# ═══════════════════════════════════════════════════════════════
s = add_slide(); bg(s); accent_bar(s, ACCENT_PURP)
section_header_box(s, "Research Directions", "Future Work", "Six clear research directions building on MAIR+++")
...
txt(s, "MAIR+++ demonstrates that a fully local, memory-augmented, adaptive multi-agent framework can match or exceed GPU-dependent systems on key degradation types, while adding explainability, safety guarantees, and online learning capabilities.", 0.55, 5.15, 12.2, 0.6, size=13, color=TEXT_WHITE)
...
txt(s, "MAIR+++ — Memory-Augmented Adaptive Multi-Agent Image Restoration", 0.5, 5.5, 12.3, 0.35, size=14, color=TEXT_MUTED, align=PP_ALIGN.CENTER)
